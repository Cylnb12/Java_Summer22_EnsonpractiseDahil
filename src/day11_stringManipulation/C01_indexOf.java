package day11_stringManipulation;

public class C01_indexOf {
    public static void main(String[] args) {

        String str="Java ogrenmek cok guzel";
        // Verilen bir string de herhangi bir string yada charin
        // ilk kullanildigi index i bize dondurur


        System.out.println(str.indexOf('g'));//6
        System.out.println(str.indexOf("r"));//7

        System.out.println(str.indexOf("mek"));//10

        System.out.println(str.indexOf("g",6));// 6. index ten itibaren demek: 6 dahil
                                                            // 6 dan sonrasi icin 7 kullan
        //yukardaki 2.ve3. enin indexlerini bulun
        //2. e yi bulabilmek icin 1. e nin indexine ihtiyacim var

        int ilke=str.indexOf('e');

        int ikincie=str.indexOf("e",ilke+1);

        //eger 2.e varsa 3.e nin olup olmadigini
        //ve varsa index ini yazdiralim

        if (ikincie==-1){
            System.out.println("verilen str de ikinci e yok");
        }else {
            int ucuncue=str.indexOf("e", ikincie+1);
            if (ucuncue==-1){
                System.out.println("verilen str de ucuncu e yok");
            }else {
                System.out.println("verilen str deki ucuncu e nin index i : "+ucuncue);
            }
        }



    }
}
